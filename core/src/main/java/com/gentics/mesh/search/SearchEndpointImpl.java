...
public class SearchEndpointImpl extends AbstractInternalEndpoint implements SearchEndpoint {
	...
	private void addSearchEndpoints() {
		registerHandler("users", (uuid) -> boot.get().userDao().findByUuidGlobal(uuid), UserListResponse.class, userSearchHandler,
			userExamples.getUserListResponse(), false);
		registerHandler("groups", (uuid) -> boot.get().groupDao().findByUuidGlobal(uuid), GroupListResponse.class, groupSearchHandler,
			groupExamples.getGroupListResponse(), false);
		registerHandler("roles", (uuid) -> boot.get().roleDao().findByUuidGlobal(uuid), RoleListResponse.class, roleSearchHandler,
		...
		registerHandler("tagFamilies", (uuid) -> boot.get().tagFamilyDao().findByUuidGlobal(uuid), TagFamilyListResponse.class,
			tagFamilySearchHandler,
			tagFamilyExamples.getTagFamilyListResponse(), false);

		registerHandler("projects", (uuid) -> boot.get().projectDao().findByUuidGlobal(uuid), ProjectListResponse.class,
			projectSearchHandler, projectExamples
				.getProjectListResponse(),
			false);
		registerHandler("schemas", (uuid) -> boot.get().schemaDao().findByUuidGlobal(uuid), SchemaListResponse.class,
			schemaContainerSearchHandler,
			schemaExamples.getSchemaListResponse(), false);
		registerHandler("microschemas", (uuid) -> boot.get().microschemaDao().findByUuidGlobal(uuid), MicroschemaListResponse.class,
