...
public class SchemaTest extends AbstractMeshTest implements BasicObjectTestcases {
	...
	public void testDefaultSchema() {
		try (Tx tx = tx()) {
			SchemaDao root = boot().schemaDao();
			assertEquals(schemaContainers().size(), root.globalCount());
		}
	}
	...
	public void testFindAllVisible() throws InvalidArgumentException {
		try (Tx tx = tx()) {
			Page<? extends HibSchema> page = boot().schemaDao().findAll(mockActionContext(), new PagingParametersImpl(1, 25L));
			assertNotNull(page);
		}
	}
	...
	public void testFindAll() throws InvalidArgumentException {
		try (Tx tx = tx()) {
			int size = Iterables.size(boot().schemaDao().findAllGlobal());
	...
	public void testFindByUUID() throws Exception {
		try (Tx tx = tx()) {
			...
			assertNotNull("The schema could not be found", boot().schemaDao().findByUuidGlobal(uuid));
		}
	}
	...
	public void testDelete() throws Exception {
		try (Tx tx = tx()) {
			...
			assertNull("The schema should have been deleted", boot().schemaDao().findByUuidGlobal(uuid));
		}
	}
	...
	public void testCreateDelete() throws Exception {
		try (Tx tx = tx()) {
			...
			assertNull("The container should have been deleted", boot().schemaDao().findByUuidGlobal(uuid));
		}
	}
