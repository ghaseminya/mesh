...
public class TagEndpointTest extends AbstractMeshTest implements BasicObjectTestcases {
	...
	public void testFindAllVisible() throws InvalidArgumentException {
		try (Tx tx = tx()) {
			TagFamilyDao root = boot().tagFamilyDao();
			root.findAll(project(), mockActionContext(), new PagingParametersImpl(1, 10L));
		}
	}
	...
	public void testFindAll() throws InvalidArgumentException {
		try (Tx tx = tx()) {
			TagFamilyDao root = boot().tagFamilyDao();
			List<? extends HibTagFamily> families = root.findAllGlobal().list();
			...
			assertEquals(3, root.globalCount());
		}
	}
	...
	public void testFindByUUID() throws Exception {
		try (Tx tx = tx()) {
			...
			HibTagFamily foundTagFamily = tagFamilyDao.findByUuidGlobal(tagFamily.getUuid());
	...
	public void testCreateDelete() throws Exception {
		try (Tx tx = tx()) {
			...
			HibTagFamily foundTagFamily = tagFamilyDao.findByUuidGlobal(uuid);
			assertNotNull(foundTagFamily);
			BulkActionContext bac = createBulkContext();
			tagFamilyDao.delete(tagFamily, bac);
			// TODO check for attached nodes
			HibProject project = boot().projectDao().findByUuidGlobal(uuid);
			assertNull(project);
		}
